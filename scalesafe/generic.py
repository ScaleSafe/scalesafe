...
class GenericMonitor():
    ...
    def _checkExceptions(self, ss_response):
        ...
        if "invalid api key" in error_message:
            ...
            raise DataValidationError("Data validation error: missing or incorrect fields.")
        elif "unauthorized" in error_message:
            raise UnauthorizedAccessError("Unauthorized access attempt detected.")
        elif "model not found" in error_message or "resource not found" in error_message:
    ...
    def _get_api_key(self, api_key_passed=None):
        ...
        if os_api_key:
            return os_api_key
        raise ScaleSafeTokenError("Scalesafe API Key not found in local environment. Please set the environment variable SCALESAFE_API_KEY or pass the api_key as an argument to the monitor method.")
    ...
    def status(self, api_key=None):
        ...
        if status.get('status') == 'Out of Compliance':
            raise OutOfComplianceError(status.get('message', "Scalesafe server is showing system is out of compliance."))
        return status
